...
//!
//! Same layout as `plus.rs`: ChunkStep impls, then async dispatch (canonical),
//! then sync shims via `block_on`.
...
pub struct SubI64VecVec<'a> { x: &'a [i64], y: &'a [i64], out: &'a mut [i64], off: usize, chunk: usize }
...
pub struct SubScalarVecI64<'a> { y: &'a [i64], s: i64, out: &'a mut [i64], off: usize, chunk: usize }
impl<'a> ChunkStep for SubScalarVecI64<'a> {
    fn step(&mut self) -> Option<usize> {
...
pub struct SubVecScalarI64<'a> { x: &'a [i64], s: i64, out: &'a mut [i64], off: usize, chunk: usize }
impl<'a> ChunkStep for SubVecScalarI64<'a> {
    fn step(&mut self) -> Option<usize> {
        if self.off >= self.x.len() { return None; }
        let end = (self.off + self.chunk).min(self.x.len());
...
pub async unsafe fn minus_i64_i64_async(x: RefObj, y: RefObj, ctx: &Ctx<'_>) -> Result<RefObj, KernelErr> {
    let chunk = if ctx.chunk_elems != 0 { ctx.chunk_elems } else { I64_CHUNK };
    match (x.is_atom(), y.is_atom()) {
        (true, true) => Ok(alloc_atom(Kind::I64, x.atom::<i64>().wrapping_sub(y.atom::<i64>()))),
        (true, false) => {
            let s = x.atom::<i64>();
            ...
            let mut out = alloc_vec_i64(ctx, ys.len() as i64);
            {
                ...
                let mut k = SubScalarVecI64 { y: ys, s, out: os, off: 0, chunk };
        ...
        (false, true) => {
            ...
            {
                let os = out.as_mut_slice::<i64>();
                let mut k = SubVecScalarI64 { x: xs, s, out: os, off: 0, chunk };
                drive_async(&mut k, ctx).await?;
            }
            if (x.attr() & attr_flags::HAS_NULLS) != 0 {
                let os = out.as_mut_slice::<i64>();
                for i in 0..xs.len() { if xs[i] == NULL_I64 { os[i] = NULL_I64; } }
                out.set_attr(attr_flags::HAS_NULLS);
            }
            Ok(out)
        }
        (false, false) => {
                ...
                && xs.len() >= parallel::PARALLEL_THRESHOLD;
            {
                ...
                if go_parallel {
                    ...
                    let mut k = SubI64VecVec { x: xs, y: ys, out: os, off: 0, chunk };
                    drive_async(&mut k, ctx).await?;
                    if has_nulls {
                        for i in 0..xs.len() {
...
pub unsafe fn minus_i64_i64(x: RefObj, y: RefObj, ctx: &Ctx) -> Result<RefObj, KernelErr> {
    block_on(minus_i64_i64_async(x, y, ctx))
}

// ---- F64 ChunkStep impls ----

pub struct SubF64VecVec<'a> { x: &'a [f64], y: &'a [f64], out: &'a mut [f64], off: usize, chunk: usize }
impl<'a> ChunkStep for SubF64VecVec<'a> {
    fn step(&mut self) -> Option<usize> {
        if self.off >= self.x.len() { return None; }
        let end = (self.off + self.chunk).min(self.x.len());
...
pub struct SubScalarVecF64<'a> { y: &'a [f64], s: f64, out: &'a mut [f64], off: usize, chunk: usize }
impl<'a> ChunkStep for SubScalarVecF64<'a> {
    fn step(&mut self) -> Option<usize> {
        if self.off >= self.y.len() { return None; }
        let end = (self.off + self.chunk).min(self.y.len());
...
pub struct SubVecScalarF64<'a> { x: &'a [f64], s: f64, out: &'a mut [f64], off: usize, chunk: usize }
impl<'a> ChunkStep for SubVecScalarF64<'a> {
    fn step(&mut self) -> Option<usize> {
        if self.off >= self.x.len() { return None; }
        let end = (self.off + self.chunk).min(self.x.len());
...
pub async unsafe fn minus_f64_f64_async(x: RefObj, y: RefObj, ctx: &Ctx<'_>) -> Result<RefObj, KernelErr> {
    ...
    match (x.is_atom(), y.is_atom()) {
        ...
        (true, false) => {
            ...
            let mut out = alloc_vec_f64(ctx, ys.len() as i64);
            {
                ...
                let mut k = SubScalarVecF64 { y: ys, s, out: os, off: 0, chunk };
                drive_async(&mut k, ctx).await?;
            }
        ...
        (false, true) => {
            ...
            {
                let os = out.as_mut_slice::<f64>();
                let mut k = SubVecScalarF64 { x: xs, s, out: os, off: 0, chunk };
                drive_async(&mut k, ctx).await?;
            }
        ...
        (false, false) => {
                ...
                && xs.len() >= parallel::PARALLEL_THRESHOLD;
            {
                ...
                if go_parallel {
                    ...
                    let mut k = SubF64VecVec { x: xs, y: ys, out: os, off: 0, chunk };
                    drive_async(&mut k, ctx).await?;
                }
            }
...
pub unsafe fn minus_f64_f64(x: RefObj, y: RefObj, ctx: &Ctx) -> Result<RefObj, KernelErr> {
    block_on(minus_f64_f64_async(x, y, ctx))
}
